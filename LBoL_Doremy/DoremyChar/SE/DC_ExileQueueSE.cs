...
{
    public abstract class DC_ExileQueueSE : DStatusEffect
    {
        ...
        protected string GetQueuedCardsDesc(IEnumerable<Card> queue)
        {
            if (queue.FirstOrDefault() == null)
                return DC_ExileQueueTooltipSE.GetSharedLoc("Nothing");
            var rez = string.Join(", ", queue.Take(maxQDisplay).Select(c => c.ColorName()));

            var count = queue.Count();
            if (count > maxQDisplay)
            {
                rez += DC_ExileQueueTooltipSE.GetSharedLoc("OverQ", new object[] { count - maxQDisplay });
            }
        ...
        protected virtual string GetNoTargetCardInBattle() => DC_ExileQueueTooltipSE.GetSharedLoc("NotInExile");

        protected virtual void PostProcessCopy(Card copy) { return; }

        protected IEnumerable<BattleAction> ProcessQueue(IList<Card> queue)
        {
            ...
            for (int i = 0; i < queue.Count; i++)
            {
                    ...
                    || (card.CardType == LBoL.Base.CardType.Friend && card.Summoned))
                {
                    ...
                    if (!moveBroken)
                    {
                        if (card.Battle != null && card.Zone != CardZone.Hand)
                        {
                        ...
                        {
                            yield return PerformAction.Chat(Owner, string.Format(GetNoTargetCardInBattle(), card.ColorName()), 3f, talk: false);
                            actuallyToRemove.Add(i);
                        }
                        UpdateCount(queue, actuallyToRemove.Count);
                    }
